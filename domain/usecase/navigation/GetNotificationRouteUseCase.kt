    ...
    private fun resolveRoute(eventType: NotificationEventType, data: NotificationData): String? {
        return when (eventType) {
            ...
            NotificationEventType.MESSAGE_REQUEST_RESULT -> {
                if (data.status == NotificationEventStatus.ACCEPTED && data.roomId != null) {
                    // Screen.Chat.createRoute(data.roomId)
                    null
                } else {
                    val reqId = data.requestId ?: return null
                    // Screen.RequestDetail.createRoute(reqId)
                    null
                }
            }

            NotificationEventType.CONFESSION -> Screen.Home.route
